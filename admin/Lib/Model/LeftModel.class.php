<?php
class LeftModel extends Action{ 
	//getlbls getmdlsbylbid getmdobymdmk
	//############test
	public function setleft($usrid,$mdmk){
		$info=collectinfo(__METHOD__,'$usrid,$mdmk',array($usrid,$mdmk));
		####
		$lb=D('Lb');$md=D('Md');$rbac=D('RBAC');
		...
		$lblsnw=array();
		foreach($lbls as $lbv){
			$arr_mdls=$md->getmdlsbylbid($lbv['lbid']);
			$mdls=$arr_mdls['data'];
			//1 权限判断 2 哪个页面开那个
			$mdlsnw=array();
			$dspl=0;
