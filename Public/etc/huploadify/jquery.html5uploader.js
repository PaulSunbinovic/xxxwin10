...
$.fn.html5uploader = function(opts){
	...
	var defaults = {
		...
		removeTimeout: 999999,//上传完成后进度条的消失时间
	...
	this.each(function(){
		...
		inputstr += '/>';
		inputstr += '<a href="javascript:void(0)" class="btn btn-success btn-sm" id="uploadfilebtn">';
		inputstr += option.buttonText;
		inputstr += '</a>';
		var fileInputButton = $(inputstr);
		var uploadFileList = $('<ul class="filelist" style="display:none"></ul>');
			  ...
			  funUploadFile: function(file) {
				  ...
				  (function(file) {
					  ...
					  if (xhr.upload) {
						  ...
						  //给file.name改名，使其不拥有中文名//上传的实体和xml中的姓名，至于原来的名字没用到
						  ...
						  //###########自创
						  xhr.setRequestHeader("X_FILENAME", 'tmp.'+tmp[tmp.length-1]);
						  xhr.send(file);
					  }	
				  })(file);	
